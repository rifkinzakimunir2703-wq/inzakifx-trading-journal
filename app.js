...
async function enterPublic(){
  ...
  // Ambil SATU pemilik yang memang ditetapkan untuk mode View Only.
  // Jangan pernah membaca semua user/trade di mode publik.
  const {data,error}=await sb
    .from("portal_public_owner")
    .select("owner_id")
    .eq("id",true)
    .maybeSingle();

  if(error){
    console.error("Gagal membaca portal_public_owner:",error);
    publicOwnerId=null;
  }else{
  ...
  if(publicOwnerId){
    // Query hanya data milik owner View Only.
    await loadTrades();
    await loadAccounts();
    await loadPayouts();
  }else{
    console.warn("portal_public_owner belum memiliki owner_id.");
    render();
    renderPerformance();
    renderCalendar();
  }

  // Publik hanya boleh: Performance, Calendar, Trading Journal.
  setPublicNav(false);
...
async function enterApp(user){
  ...
  // Admin selalu membaca data miliknya sendiri.
  await loadTrades();
  await loadProp();
  await loadAccounts();
  await loadPayouts();

  // Semua menu dan kontrol pengelolaan hanya aktif setelah login.
  setPublicNav(true);
  openPage("globalPage");
}
...
function render(){
  ...
  $("tradeList").innerHTML=filtered.slice().reverse().map(t=>`
    <div class="trade">
      ...
      ${currentUser ? `<button class="danger" onclick="deleteTrade('${t.id}')">Hapus</button>` : ""}</div>
